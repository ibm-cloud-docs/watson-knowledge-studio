...
- Evite menções de sobreposição, que são dois rótulos de tipo de entidade diferentes que são aplicados a uma única frase em um documento. Por exemplo, dada a sentença, *Ela doou as revistas de seu pai para a Biblioteca JFK.*, você poderia sobrepor menções se anotasse JFK=PERSON e JFK Library=LOCATION para a única frase *Biblioteca JFK*. O uso do termo é mais sobre a biblioteca que a pessoa nesta sentença, então somente a última anotação deve ser aplicada.
...
- Determine como a equipe manipulará as listas e os plurais no texto em execução. Por exemplo, o sistema de tipos KLUE tem os tipos de entidade PERSON e PEOPLE que distinguem o singular do plural. É possível escolher anotar a lista *Barack, Michelle, Malia e Sasha Obama* de uma das maneiras a seguir:
    ...
    - Anote cada item na lista como uma menção de entidade singular (*Barack*, *Michelle*, *Malia* e *Sasha Obama* são cada uma menção PERSON)
    - Anote a frase inteira como uma menção de entidade plural (*Barack, Michelle, Malia e Sasha Obama* é uma única menção PEOPLE).

    Nenhuma abordagem é necessariamente melhor que a outra; mas certifique-se de que sua equipe escolha uma delas e aplique-a consistentemente a quaisquer listas que ocorrem nos documentos.
...
- **Modo de relação**
    ...
    Nesse modo, o anotador humano conecta menções associando um tipo de relação, conforme definido no sistema de tipos. Por exemplo, a menção John Smith pode ser conectada à menção {{site.data.keyword.IBM_notm}} pelo tipo de relação employedBy. A anotação de tipos de relação é opcional e pode ocorrer antes ou depois de você anotar menções como correferências.

- **Modo de correferência**

    Nesse modo, o anotador humano identifica menções que significam a mesma coisa, portanto, ajudando a assegurar a consistência nas anotações quando palavras não são idênticas. Por exemplo, a menção de {{site.data.keyword.IBM_notm}} na primeira sentença, a menção de International Business Machines e a menção de {{site.data.keyword.IBM_notm}} em uma sentença mais recente referem-se à mesma coisa e todas seriam rotuladas pelo mesmo tipo de entidade, como ORGANIZATION. A anotação de menções como correferências é opcional e pode ocorrer antes ou depois de você anotar tipos de relação.

### Dicas para usar o editor

- Salve seu trabalho conforme você avança.
- Se você cometer um erro, será possível pressionar `Ctrl+Z` para desfazer a ação anterior. Para refazer a ação depois de desfazê-la, pressione `Ctrl+Y`. É possível desfazer as 10 ações prévias que você executou ao editar o documento atual; elas são perdidas assim que o documento é fechado. As ações devem ser desfeitas em ordem inversa e deve-se alternar para o modo em que estava quando executou a ação para desfazê-la. Não é possível desfazer e refazer ações da ferramenta de concordância.
...
- Se você escolheu usar um tokenizer baseado em dicionário com a área de trabalho e deseja manipular termos compostos e pontuação em seus dados de treinamento, é possível incluir os termos em um dicionário e criar um anotador de dicionário para pré-anotar as ocorrências. Por exemplo, para evitar que o limite de sentença quebre termos que incluem pontuação, inclua termos como Yahoo! e Dr. em um dicionário. Da mesma forma, se seus dados de treinamento incluem palavras com hífen ou acrônimos alfanuméricos, como Hi-C ou MS-60-70, inclua esses termos no dicionário. Para anotar as ocorrências, independentemente do caso, inclua os termos em minúsculas (como hi-c). Para anotar variações, inclua as variações como formas superficiais (MS-60-70 e MS 60 70). **Importante**: não use essa abordagem se você estiver usando o tokenizer padrão.
...
1. Abra uma área de trabalho e, em seguida, abra a tarefa na qual você deseja trabalhar. Os conjuntos de anotações que são designados a você são exibidos.
1. Clique em um documento para abri-lo. Por padrão, o documento abre **Anotação de documento** > **Menção**, que é o modo que você usa para anotar menções de entidade.
1. Para anotar uma menção de entidade:
    ...
    1. Evite criar menções de sobreposição. Mas se uma menção de sobreposição válida for necessária, clique em **Substituir** para incluí-la mais facilmente. Uma sobreposição ocorre quando você aplica mais de um rótulo a uma menção de entidade. Revise estas sugestões:
        ...
        - Anote Sub-Saara como uma única menção, não apenas Saara ou apenas Sub.
        - Não crie uma anotação PERSON de sobreposição para a referência *JFK* em *Aeroporto Internacional JFK*. A menção inteira *Aeroporto Internacional JFK* deve ser rotulada somente como FACILITY.
    ...
    1. Para remover uma anotação que você acabou de incluir, pressione `Ctrl+Z` para desfazer a ação. Para remover uma menção de entidade posteriormente, é possível clicar com o botão esquerdo do mouse em uma menção e pressionar a tecla **Delete** ou clicar em **Visualizar detalhes** e, em seguida, clicar no **X** ao lado do tipo de entidade designado à menção.
...
1. Abra uma área de trabalho e, em seguida, abra a tarefa na qual você deseja trabalhar. Os conjuntos de anotações que são designados a você são exibidos.
1. Clique em um documento para abri-lo. Por padrão, o documento abre **Anotação de documento** > **Menções**.
...
1. Selecione os documentos aos quais você deseja aplicar o tipo de entidade selecionado. É possível criar as anotações em todos os documentos que você foi designado para anotar, todos os documentos que você começou a anotar ou todos os documentos que ainda não começou a anotar. Clique em **Visualização** para ver as anotações que serão incluídas.
1. Se você deseja visualizar as anotações em contexto maior, clique nos ícones para visualizar o conteúdo do documento ou abra o documento em uma nova janela.
...
O mais importante é a consistência. Decida sobre como você deseja anotar a correferência e especificar as regras, com exemplos, claramente em suas diretrizes de anotação.

### Procedimento

Para anotar menções como correferências:

1. Clique em **Anotação de documento** > **Correferências**.
1. Para criar uma cadeia de correferência:
    ...
    1. Para destacar todas as menções em uma cadeia para revisá-las no contexto, passe o mouse sobre a cadeia na área de janela lateral.

1. A Lista de menções únicas exibe os termos no documento que foram anotados, mas não foram incluídos em uma cadeia. Se você observar uma menção na lista que pertence a uma cadeia, será possível incluí-la na cadeia daqui.
...
1. Para remover uma cadeia de correferência que você acabou de incluir, pressione `Ctrl+Z` para desfazer a ação. Para remover posteriormente uma cadeia de correferência, no painel lateral Cadeias de correferência, clique no **X** ao lado da cadeia que você deseja remover. Para remover uma única menção da cadeia, clique no ID correferência para abrir uma janela que exibe uma lista das menções na cadeia e, em seguida, clique no **X** próximo à menção que você deseja remover.
1. Clique em **Salvar** a qualquer momento para salvar o seu trabalho.
...
1. Clique em **Anotação de documento** > **Relações**.
...
1. Para remover uma menção de relação que você acabou de incluir, pressione `Ctrl+Z` para desfazer a ação. Para remover uma menção de relação posteriormente, é possível clicar com o botão esquerdo do mouse no tipo de relação e, em seguida, pressionar a tecla **Delete** ou clicar no **X** ao lado do tipo de relação.

...
1. Desde la [página Panel de control ![Icono de enlace externo](../../icons/launch-glyph.svg "Icono de enlace externo")](https://console.bluemix.net/dashboard/apps){: new_window}, regístrese para un plan de {{site.data.keyword.knowledgestudioshort}}.

  1. Pulse **Catálogo**.
  1. En el campo de búsqueda, suprima el filtro **label:lite** (si está presente) y busque `{{site.data.keyword.knowledgestudioshort}}`.
  1. Seleccione **{{site.data.keyword.knowledgestudioshort}}**.
  1. Si aún no tiene una [cuenta de Pago según uso ni una cuenta de Suscripción ![Icono de enlace externo](../../icons/launch-glyph.svg "Icono de enlace externo")](https://console.bluemix.net/docs/pricing/index.html){: new_window}, en la página de catálogo de {{site.data.keyword.knowledgestudioshort}}, pulse **Actualizar**. Se le solicitará que proporcione información de la tarjeta de crédito.

    Si elige un Plan gratuito de {{site.data.keyword.knowledgestudioshort}}, no se cargará a su tarjeta de crédito. Puede utilizar el Plan gratuito durante un tiempo ilimitado sin coste alguno.
    {: tip}

1. Después de registrarse para un plan, inicie {{site.data.keyword.knowledgestudioshort}}.

  Para completar esta guía de aprendizaje, debe tener al menos un ID de usuario que pueda utilizar en {{site.data.keyword.knowledgestudioshort}}. Este ID de usuario debe tener el rol Admin. Si se ha registrado para un plan gratuito, como el único usuario, tendrá el rol Admin. Para obtener información sobre los roles de usuario, consulte [Ensamblaje de un equipo](/docs/services/watson-knowledge-studio/team.html).
...
1. Inicie sesión en {{site.data.keyword.knowledgestudioshort}} con su ID de administrador.
1. Pulse el icono Configuración para abrir la página Detalles de servicio. La página lista todos los ID de usuario registrados como usuarios de {{site.data.keyword.knowledgestudioshort}}. Cada ID de usuario tiene uno de los siguientes roles (en orden descendente de permisos incluidos):
    ...
    Para obtener más información sobre los roles de usuario, consulte [Ensamblaje de un equipo](/docs/services/watson-knowledge-studio/team.html).
...
1. Si tiene acceso a ID de usuario adicionales, verifique que haya al menos dos usuarios con el rol HumanAnnotator.
...
1. Opcional: Cambie el rol asignado a un ID de usuario. Pulse el icono **Modificar valor de la cuenta** en la columna **Acción** de la fila de tabla para un ID de usuario y cambie el rol de usuario asignado.

    > **Nota:** Puede actualizar un ID de usuario a un rol con permisos mayores, pero no puede degradar un usuario con un rol Admin o ProjectManager al rol HumanAnnotator.
...
1. Como administrador de {{site.data.keyword.knowledgestudioshort}}, desde su {{site.data.keyword.cloud_notm}} [panel de control ![Icono de enlace externo](../../icons/launch-glyph.svg "Icono de enlace externo")](https://console.bluemix.net/dashboard/apps/){:new_window}, inicie el servicio de {{site.data.keyword.knowledgestudioshort}}.
...
1. Desde la barra lateral, pulse **Activos y herramientas** > **Tipos de entidades**.
1. En la página Tipos de entidades, pulse **Cargar**.
1. Seleccione el archivo `en-klue2-types.json` desde su sistema y pulse **Cargar**. El sistema de tipos descargado se mostrará en la tabla.
...
1. Edite un tipo de entidad:
    ...
    1. En la columna **Roles**, pulse el icono **Suprimir un rol** ![_](images/wks_delete.jpg) junto al rol AWARD.

        ![Una captura de pantalla de la supresión de un rol desde un tipo de entidad.](images/wks_tut_delete_role.jpg)
...
1. En la ventana Cargar entradas de diccionario, seleccione el archivo `dictionary-items-organization.csv` desde el sistema y luego pulse **Cargar**. Los términos del archivo se cargan en el diccionario.
...
1. Pulse **Guardar**.
    ...
    ![Una captura de pantalla de la página Diccionarios. Se selecciona una entrada de diccionario y su forma superficial y categoría léxica se resaltan.](images/wks_tutdict4.jpg)

...
1. [現行インスタンスから成果物をエクスポートする](#export)
...
1. 現行のユーザー役割のリストを作成します。
    ...
    **注:** このステップはオプションです。通常、これが実行されるのは、ワークスペースがインスタンス間でマイグレーションされ、新規ワークスペースが元のワークスペースと同じでなければならない場合です。ワークスペース・データのみを新規ワークスペースにマイグレーションする場合は、このステップをスキップできます。
    ...
    役割について詳しくは、『[チームのアセンブル](/docs/services/watson-knowledge-studio/team.html)』を参照してください。
...
- 辞書
  ...
  **注:** 編集可能辞書のみがダウンロードされます。読み取り専用辞書をダウンロードすることはできません。
...
1. 前に作成した辞書バックアップから辞書をアップロードします。
  ...
  **注:** 辞書を追加すると、辞書プレアノテーターが自動的に作成されます。このプレアノテーターを実行するときに、辞書をエンティティー・タイプと関連付けます。
...
1. 機械学習モデルをトレーニングします。詳しくは、『[機械学習モデルの作成](/docs/services/watson-knowledge-studio/train-ml.html)』を参照してください。

  **注:** このワークスペースにマイグレーションしたアノテーション付きの文書に対しては、どのプレアノテーターも実行しないでください。それを行うと、ヒューマン・アノテーターが追加したアノテーションがすべて失われることになります。
...
1. まだアノテーションが付けられていないが、モデルを改善し続けるためにグランド・トゥルースに追加したい文書をすべてアップロードします。
1. 新しくインポートされ、アノテーションが付けられていない文書から、アノテーション用の文書セットを作成します。これらのセットは_アノテーション・セット_ と呼ばれます。詳しくは、『[アノテーション・セットの作成および割り当て](/docs/services/watson-knowledge-studio/document-for-annotation.html)』を参照してください。
1. アノテーション・タスクを再作成します。タスクに同じ名前と適切な期限を指定し、適切なヒューマン・アノテーターにアノテーション・セットを割り当てます。

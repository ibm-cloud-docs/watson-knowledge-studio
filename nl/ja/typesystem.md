...
- **役割**
    ...
    これは、メンションが出現するコンテキストによってメンションを限定します。例えば、「学生たちはフランスに行った」というステートメント内の「フランス」というメンションには、エンティティー・タイプ GPE  (地政学的エンティティー) がマーク付けされます。フランスが地政学的エンティティーであるからです。しかし、このコンテキストでのフランスは、旅行する学生たちの目的地でもあるため、このエンティティー・タイプは属性 (この場合は LOCATION (場所) という役割) を追加することによって限定されます。別の例として、メンション「弁護士」には、エンティティー・タイプ PEOPLE (人々) によってマーク付けし、役割 OCCUPATION (職業) によってもマーク付けすることができます。
...
- 名前にスペースを含めることはできません。
- タイプ・システムに追加する値には、英数字の ASCII 文字 (A から Z、a から z、0 から 9) と下線文字のみを使用してください。
...
1. {{site.data.keyword.knowledgestudioshort}} の管理者またはプロジェクト管理者としてログインし、**「アセットおよびツール」**>**「エンティティー・タイプ」**ページを開きます。
1. タイプ・システムを作成する方法として、以下のいずれかを選択します。

    - 既存のタイプ・システムをアップロードする場合:

        1. **「アップロード」**をクリックして、既存のタイプ・システムをアップロードします。

            以前に {{site.data.keyword.knowledgestudioshort}} ワークスペースからタイプ・システムをダウンロードした場合、そのタイプ・システムは `JSON` 形式でダウンロードされています。このタイプ・システムをアップロードして、新規ワークスペースの作成をジャンプ・スタートさせることができます。詳しくは、『[別のワークスペースからのリソースのアップロード](/docs/services/watson-knowledge-studio/exportimport.html)』を参照してください。
    ...
    - タイプ・システムを最初から作成する場合:
        ...
        1. **「メンション・クラス」**および**「メンション・タイプ」**をクリックして、デフォルトのメンション・クラスとメンション・タイプを表示します (それらの値は編集できません)。
        ...
        1. **「アセットおよびツール」**>**「関係タイプ」**ページを開き、2 つのメンションが互いにどのような関係にあるかを指定します。
...
例として、ソース文書内に以下のような英語の文章があり、「Acme」(ここで、Acme は既知のトラック製造メーカー)、「sedan (セダン)」、および「truck (トラック)」を類似したエンティティーとしてキャプチャーしたいと想定します。それらは、どれも物理的な車両を指しているためです。

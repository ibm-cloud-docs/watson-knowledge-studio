...
이 문서는 {{site.data.keyword.knowledgestudiofull}} on {{site.data.keyword.cloud}}에 대한 문서입니다. 이전 {{site.data.keyword.knowledgestudioshort}} on {{site.data.keyword.IBM_notm}} Marketplace 버전에 대한 문서를 보려면 [이 링크 ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘")를 클릭하십시오](https://console.bluemix.net/docs/services/knowledge-studio/tutorials-create-rule-model.html){: new_window}.
{: tip}
...
1. **Create Dictionary** 단추를 클릭하여 사전을 추가하십시오. 
1. **Name** 필드에 `Month dictionary`를 입력하고 **Save**를 클릭하여 비어 있는 사전을 작성하십시오. 새 사전이 작성되며 자동으로 편집할 수 있도록 열립니다. 
1. 사전 분할창에서 **Upload**를 클릭하십시오. 
1. Upload Dictionary Entries 창에서 컴퓨터에 있는 `dictionary-items-month.csv` 파일을 선택한 후 **Upload**를 클릭하십시오. 

    파일에 있는 용어를 사전으로 가져옵니다. 
...
1. 사이드바에서 **Document Annotation** > **Rules**를 클릭하십시오. 
1. **Documents** 옆에 있는 **문서 추가** 아이콘을 클릭하십시오. 
...
1. 클릭하여 이전에 컴퓨터에 다운로드한 `documents-new.csv` 파일을 찾은 후 **Upload**를 클릭하십시오. 
    ...
    ![규칙 편집기에 추가된 14개의 문서 중 세 개가 표시되어 있습니다. 각 문서의 문서 제목과 시작 부분의 발췌가 표시되어 있습니다. 또한 각 문서 옆에는 문서를 삭제하는 데 사용할 수 있는 삭제 아이콘이 있습니다. ](images/rule-doc-add3.jpg)
...
1. 작업공간의 **Rules** 페이지에서, 오른쪽 패널의 **Class** 옆에 있는 **클래스 추가** 아이콘을 클릭하십시오. 
...
1. 사이드바에서 **Document Annotation** > **Dictionaries**을 선택한 후 이전에 작성한 **Month dictionary**를 클릭하십시오. 

1. **Class** 목록에서 `DictMonth`를 선택한 후 **Save**를 클릭하십시오. 

    클래스가 사전과 연관됩니다. 

    ![Rules 페이지의 Dictionaries 패널에서 DictMonth 클래스가 Month dictionary와 연관된 것을 보여줍니다. ](images/rule-dict-map2.jpg)
...
1. 사이드바에서 **Document Annotation** > **Rules**를 선택하십시오. 
1. Class 패널에서 이전에 정의한 `DictMonth` 클래스를 찾고 그 옆에 있는 **문서에서 어노테이션 검색** 아이콘을 클릭하십시오. 

    Find Annotations 페이지가 표시되며 월에 대한 모든 텍스트 참조를 포함하는 문서가 표시됩니다. 

1. `Technology - computerworld.com` 문서를 클릭하여 전체 문서를 보십시오. 텍스트 `February`가 강조표시된 것을 볼 수 있으며, 이는 이것이 `DictMonth` 클래스의 멘션으로 어노테이션 작성되었음을 의미합니다. 
...
1. **Rules** 페이지에서 오른쪽 패널의 **Class** 옆에 있는 **클래스 추가** 아이콘(![_](images/wks_tut_dict_add.jpg))을 클릭하십시오. 
1. 클래스 이름으로 `RegExpYear`를 입력하고 **Add**를 클릭하십시오. 
1. 사이드바에서 **Regex**를 클릭한 후 **Regular Expressions** 옆에 있는 **정규식 작성** 아이콘을 클릭하십시오. 
1. **Add Entry** 단추를 클릭하십시오. 
1. **Regular Expression** 필드에 다음 표현식을 입력하십시오. 
    ...
    > **참고:** 이 정규식은 1900 - 2099 범위의 연도를 찾습니다. 
...
1. 정의한 표현식이 시간 발생을 올바르게 캡처하고 있는지 확인하려는 경우에는 멘션을 검색할 수 있습니다. Class 사이드 패널에서 **RegExpYear** 클래스 옆에 있는 **문서에서 어노테이션 검색** 아이콘을 클릭하십시오. 

    ![Rules 페이지의 Class 사이드 패널에서 사용자가 RegExpYear 클래스 옆에 있는 돋보기 아이콘을 클릭하고 있는 것을 보여줍니다. ](images/rule-regex-add5.jpg)

    Find Annotations 페이지가 표시됩니다. 연도 멘션이 발생한 샘플 문서에 해당 발생이 강조표시되어 있습니다. 

    ![샘플 문서 중 8개의 발췌에서 강조표시된 연도를 보여줍니다. ](images/rule-regex-add6.jpg)
...
1. 사이드바에서 **Document Annotation** > **Rules**를 선택하고 `Technology - computerworld.com` 문서를 여십시오. 
1. 문서의 텍스트 *February 3, 2009*를 선택하십시오. 쉼표도 함께 선택해야 합니다. 

    ![문서에서 텍스트 February 3, 2009가 선택된 것을 보여줍니다. ](images/rule-add1.jpg)

1. **규칙 추가** 아이콘을 클릭하십시오. 

    규칙 편집기는 식별한 규칙 패턴을 이미지로 보여줍니다. 

    텍스트 *February 3, 2009*가 표시됩니다. 이미지에서 셀을 연결하는 회색 선은 현재 패턴의 일부인 셀을 식별합니다. 
...
1. 일 `3` 셀의 위에서 **텍스트** 아이콘을 클릭하여 토큰에 대한 특성 설정을 여십시오. 

    ![사용자가 '텍스트 특성 설정' 아이콘을 클릭하는 것을 보여줍니다. ](images/rule-add4.jpg)

    현재 규칙은 정확히 텍스트 `3`과 일치하도록 설정되어 있습니다. 이를 임의의 숫자와 일치하도록 해야 합니다. 

1. **Character Type : Numeric**을 선택한 후 **Text: 3**을 선택 취소하여 특성 설정을 숫자로 변경하십시오. 

    ![사용자가 Character Type : Number 옵션을 3 토큰에 대한 특성 설정으로 클릭하는 것을 보여줍니다. ](images/rule-add5.jpg)

    숫자 `3` 셀에 대한 정의를 변경했습니다. 

    ![3 토큰을 나타내는 셀 위에 패턴 내의 해당 토큰과 임의의 숫자 값이 일치할 수 있음을 표시하는 Aa 아이콘이 생긴 것을 보여줍니다. ](images/rule-add6.jpg)

    **Aa** 아이콘은 숫자가 3과 정확히 일치해야 할 필요 없이, 임의의 숫자와 일치할 수 있음을 나타냅니다. 

1. 쉼표 토큰에 대한 설정은 변경하지 마십시오. 

    패턴의 세 번째 토큰을 쉼표로 할 것이므로, 현재 설정인 **Text: ,**가 적절합니다. 특성 설정 외에, 각 토큰에는 반복 설정이 있습니다. 반복 설정은 토큰이 텍스트에서 얼마나 반복되어도 패턴과 일치할 수 있는지 지정합니다. 현재 반복 설정은 **Required (Exactly 1)**이며 이 또한 적절히 지정되었습니다. 

    ![Exactly 1로 설정된, 쉼표 토큰에 대한 반복 설정을 보여줍니다. ](images/rule-add7.jpg)

1. 패턴 `DictMonth + numeric token + comma + RegExpYear`를 나타내도록 클래스를 지정하십시오. 
    ...
    ![맨 위 행의 네 개 셀이 모두 선택되어 이 범위가 RuleDate 클래스로 정의되는 것을 보여줍니다. ](images/rule-add8.jpg)

    규칙의 패턴을 정의했습니다. 

1. **Rule name** 필드에 `MyDateRule`을 입력하고 **Save**를 클릭하십시오. 
    ...
    ![텍스트 February 3, 2009만 RuleDate 클래스의 멘션으로 어노테이션 작성되어 있는, Technology - computerworld.com 문서의 텍스트를 보여줍니다. ](images/rule-add10.jpg)

    Class 패널의 `RuleDate` 클래스 옆에 있는 **문서에서 어노테이션 검색** 아이콘(![검색을 나타내는 돋보기](images/filter_gray.png))을 클릭하여 샘플 문서에서 RuleDate 클래스 멘션의 모든 발생을 검색할 수 있습니다. 패턴을 올바르게 정의했는지 확인하기 위해 모든 날짜가 올바르게 캡처되었는지 확인하는 것이 좋습니다. 

    ![방금 정의한 규칙 패턴과 일치하는 날짜를 포함하는 두 문서가 있는 Find Annotations 페이지를 보여줍니다. ](images/rule-add11.jpg)
...
1. 사이드바에서 **Model Management** > **Versions**를 선택하고 **Rule-based model type mapping** 탭을 클릭하십시오. 

1. 정의한 `RuleDate` 클래스를 유형 시스템의 해당 `DATE` 엔티티에 맵핑하십시오. 

    1. **DATE** 엔티티를 찾고 **Edit**를 클릭하십시오. 

        ![사용자가 Annotator Component 페이지의 Class mapping 탭에서 DATE 엔티티 유형에 대해 Edit를 클릭하는 것을 보여줍니다. ](images/rule-anno2.jpg)

    1. 드롭 다운 목록에서 `RuleDate` 클래스를 선택하고 **Save**를 클릭하십시오. 

        ![사용자가 드롭 다운 목록에서 RuleDate 클래스를 선택하는 것을 보여줍니다. ](images/rule-anno3.jpg)

1. 규칙 기반 모델을 실행하려면 **Rule-based** 탭을 선택하고 **Run this model**을 클릭하십시오. 

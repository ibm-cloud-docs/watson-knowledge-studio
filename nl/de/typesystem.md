...
Viele Branchen, z. B. in den Fachgebieten Metallurgie, Geologie, Marktforschung, Biowissenschaften, elektronische Patientenaktem und Onkologie veröffentlichen Wörterlisten oder Ontologien mit fachspezifischer Terminologie. Ziehen Sie das Referenzieren dieses Ressourcentyps in Betracht, um eine Vorstellung davon zu bekommen, welche Entitätstypen Sie in Ihrem eigenen Typsystem definieren sollten.
...
- **Erwähnungstyp**
    ...
    - PRO: Die Erwähnung ist ein Pronomen (z. B. 'er', 'wir' oder 'es').
...
- **Erwähnungsklasse**
    ...
    - SPZ: Es handelt sich um eine spezifische Erwähnung (im Deutschen häufig am Artikel 'der/die/das' erkennbar, z. B. 'das Buch' oder 'der Hurricane wütete im September'). In diesen Beispielen würden die Erwähnungen 'Buch' und 'Hurricane' mit dem Attribut SPC annotiert.
...
- Verwenden Sie nur die folgenden alphanumerischen ASCII-Zeichen und das Unterstreichungszeichen in Werten, die Sie zum Typsystem hinzufügen: A bis Z, a bis z, 0 bis 9.
...
1. Melden Sie sich als {{site.data.keyword.knowledgestudioshort}}-Administrator oder -Projektleiter an und öffnen Sie die Seite **Assets & Tools** > **Entitätstypen**.
1. Verwenden Sie eine der folgenden Methoden zum Erstellen des Typsystems.

    - So laden Sie ein vorhandenes Typsystem hoch:

        1. Klicken Sie auf **Hochladen**, um ein vorhandenes Typsystem hochzuladen.

            Wenn Sie zuvor ein Typsystem aus einem {{site.data.keyword.knowledgestudioshort}}-Arbeitsbereich heruntergeladen haben, wird das Typsystem im `JSON`-Format heruntergeladen. Sie können dieses Typsystem hochladen, um die Erstellung eines neuen Arbeitsbereichs zu beschleunigen. Details hierzu finden Sie unter [Ressourcen aus einem anderen Arbeitsbereich hochladen](/docs/services/watson-knowledge-studio/exportimport.html).
    ...
    - So erstellen Sie ein Typsystem völlig neu:
        ...
        1. Klicken Sie auf **Erwähnungsklassen** und **Erwähnungstypen**, um die Standardklassen und -typen für Erwähnungen anzuzeigen (diese Werte können nicht bearbeitet werden).
        ...
        1. Öffnen Sie die Seite **Assets & Tools** > **Beziehungstypen** und geben Sie an, in welcher Beziehung zwei Erwähnungen zueinander stehen können.

            In der Regel ist die Reihenfolge zwischen der ersten und der zweiten Entität in einem Beziehungstyp von Bedeutung. Eine Entität PERSON kann beispielsweise Mitarbeiter einer Entität ORGANISATION oder eine geopolitische Entität (GPE) sein (z. B. 'Mary beschäftigtBei {{site.data.keyword.IBM_notm}}'), aber Organisationen und geopolitische Entitäten können nicht Mitarbeiter einer Person sein. Wenn ein Annotatorbenutzer im Ground Truth-Editor auf eine Entität klickt, wird die Liste der verfügbaren Beziehungstypen durch das definierte Typsystem gesteuert.
...
1. Definieren Sie nur die beiden Entitätstypen **HERSTELLER** und **FAHRZEUG** ohne Rollen.
    ...
    - c) Die Erwähnung 'Acme' wird mit dem Entitätstyp **HERSTELLER** annotiert und die Erwähnung 'Lastwagen' mit dem Entitätstyp **FAHRZEUG**.
